...
void object::test<14>()
{
    ...
    ensure_equals("y value position", getConstant(*getVariableDecl(tree->functions[0].stmts[1]).value).position, at(4, 13));
}
...
void object::test<17>()
{
    ...
    ensure_equals("b position", getIdentifier(*getVariableDecl(tree->functions[0].stmts[0]).value).position, at(3, 13));
...
void object::test<20>()
{
    ...
    ENSURE_EQUALS(getConstant(getReturn(tree->functions[0].stmts[0]).value).position, at(3, 12));
    ...
    ENSURE_EQUALS(getIdentifier(getReturn(tree->functions[0].stmts[1]).value).position, at(4, 12));
}
